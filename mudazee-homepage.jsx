...
const PRODUCTS = [
  ...
  },
  {
    ...
    reviews: 189,
    image:
      "https://images.unsplash.com/photo-1595425964272-6b3159efe4c1?w=600&q=80&auto=format&fit=crop",
    badge: "sale",
    categories: ["new", "men"],
  },
  {
  ...
  },
  {
    ...
    reviews: 214,
    image:
      "https://images.unsplash.com/photo-1608528577891-eb055944f2e7?w=600&q=80&auto=format&fit=crop",
    badge: "sale",
    categories: ["bestseller", "men"],
  },
  {
    ...
    reviews: 176,
    image:
      "https://images.unsplash.com/photo-1600612253971-422e7f7faeb6?w=600&q=80&auto=format&fit=crop",
    badge: null,
    categories: ["bestseller", "men"],
  },
  {
  ...
  },
  {
    ...
    reviews: 143,
    image:
      "https://images.unsplash.com/photo-1615368144592-05aa4ba51454?w=600&q=80&auto=format&fit=crop",
    badge: "soldout",
    categories: ["bestseller", "women"],
  },
  {
    ...
    reviews: 221,
    image:
      "https://images.unsplash.com/photo-1615529182904-14819c35db37?w=600&q=80&auto=format&fit=crop",
    badge: "sale",
    categories: ["women"],
  },
  {
    ...
    reviews: 187,
    image:
      "https://images.unsplash.com/photo-1587017539504-67cfbddac569?w=600&q=80&auto=format&fit=crop",
    badge: null,
    categories: ["women"],
  },
  {
    ...
    reviews: 267,
    image:
      "https://images.unsplash.com/photo-1621172533935-cbe0f77bb0f0?w=600&q=80&auto=format&fit=crop",
    badge: "sale",
    categories: ["women"],
  },
  {
    id: "p14",
    title: "Discovery",
    tag: "Impression of Black Orchid — Tom Ford",
    price: 2600,
    originalPrice: null,
    rating: 4.5,
    reviews: 112,
    image:
      "https://images.unsplash.com/photo-1580870069867-74c57ee1bb07?w=600&q=80&auto=format&fit=crop",
    badge: null,
    categories: ["women"],
  },
];
...
const SEASONS = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=500&q=80&auto=format&fit=crop",
  },
  {
    name: "Winter",
    note: "Amber & warm musk",
    image:
      "https://images.unsplash.com/photo-1483664852095-d6cc6870702d?w=500&q=80&auto=format&fit=crop",
  },
  {
    name: "Spring",
    note: "White florals",
    image:
      "https://images.unsplash.com/photo-1522383225653-ed111181a951?w=500&q=80&auto=format&fit=crop",
  },
  {
    name: "Fall",
    note: "Spice & vetiver",
    image:
      "https://images.unsplash.com/photo-1476820865390-c52aeebb9891?w=500&q=80&auto=format&fit=crop",
  },
];
...
function ProductCard({ product }) {
  const isSoldOut = product.badge === "soldout";
  return (
    <div className="group flex-shrink-0 w-[168px] sm:w-[210px]">
      ...
      <div className="pt-3 space-y-1">
        <h3 className="text-[13px] font-semibold text-[#141311] tracking-tight">
          {product.title}
        </h3>
        <div className="flex items-center gap-1.5">
          <Stars rating={product.rating} />
          <span className="text-[11px] text-[#8A8072]">({product.reviews})</span>
        </div>
        <p className="text-[11px] text-[#8A8072] leading-snug">{product.tag}</p>
...
function ProductShelf({ id, title, products }) {
  return (
    <section id={id} className="px-5 sm:px-8 py-10 border-t border-[#EDE9DF]">
      ...
      <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
        {products.map((p) => (
...
export default function MudazeeHome() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [cartCount] = useState(2);

  const newArrivals = PRODUCTS.filter((p) => p.categories.includes("new"));
  const bestSellers = PRODUCTS.filter((p) => p.categories.includes("bestseller"));
  const mens = PRODUCTS.filter((p) => p.categories.includes("men"));
  const womens = PRODUCTS.filter((p) => p.categories.includes("women"));

  return (
    <div className="min-h-screen bg-white text-[#141311] font-[Inter]">
      {/* Google Fonts */}
      <style>{`
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="h-16 grid grid-cols-3 items-center">
            <div className="flex items-center gap-4">
              ...
              <button className="hidden sm:flex text-[#141311]" aria-label="Search">
                <Search size={18} strokeWidth={1.5} />
              </button>
            </div>

            <div className="flex justify-center">
              <span className="font-display text-[22px] sm:text-[26px] tracking-[0.18em] font-medium">
                MUDAZEE
              </span>
            </div>

            <div className="flex items-center justify-end gap-4 sm:gap-5">
              <button className="sm:hidden text-[#141311]" aria-label="Search">
                <Search size={18} strokeWidth={1.5} />
              </button>
              <button className="hidden sm:flex text-[#141311]" aria-label="Account">
                <User size={18} strokeWidth={1.5} />
              </button>
              <button className="relative text-[#141311]" aria-label="Bag">
                <ShoppingBag size={18} strokeWidth={1.5} />
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-[#A32424] text-white text-[9px] font-semibold w-4 h-4 rounded-full flex items-center justify-center">
                    {cartCount}
                  </span>
                )}
              </button>
        ...
        <div className="mx-auto max-w-7xl relative bg-[#141311] overflow-hidden">
          <div className="grid sm:grid-cols-2 items-stretch min-h-[360px] sm:min-h-[440px]">
            <div className="flex flex-col justify-center px-6 sm:px-12 py-10 sm:py-0 z-10">
              ...
              <button className="w-fit bg-white text-[#141311] text-[12px] font-semibold tracking-wide px-6 py-3 hover:bg-[#C7A96A] hover:text-white transition-colors">
                Shop the sale
              </button>
            </div>
      ...
      {/* ---------- PRODUCT SHELVES ---------- */}
      <ProductShelf id="new-arrivals" title="New Arrival Sale" products={newArrivals} />
      <ProductShelf title="Best Selling Perfumes" products={bestSellers} />
      <ProductShelf id="mens" title="Men's Perfume" products={mens} />
      <ProductShelf id="womens" title="Women's Perfume" products={womens} />
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8 py-14 grid grid-cols-1 sm:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <form
              ...
              className="flex items-center border border-[#333] focus-within:border-[#C7A96A]"
            >
              <input
                type="email"
              ...
              />
              <button type="submit" className="px-3 text-white" aria-label="Subscribe">
                <Send size={15} strokeWidth={1.5} />
              </button>
            </form>
            <div className="flex items-center gap-3 mt-5">
              <Instagram size={16} strokeWidth={1.5} className="hover:text-white transition-colors" />
              <Facebook size={16} strokeWidth={1.5} className="hover:text-white transition-colors" />
            </div>
          </div>
        </div>
